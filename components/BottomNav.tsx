...
import { IconHome, IconMegaphone, IconBallot, IconCard, IconUser } from './Icons';
      ...
      {tabs.map(({ href, label, Icon }) => {
        ...
        return (
          <Link key={href} href={href} className="flex flex-col items-center gap-0.5 px-2">
            <Icon width={20} height={20} className={active ? 'text-brand' : 'text-ink-muted'} />
            <span className={`text-[10.5px] ${active ? 'text-brand font-medium' : 'text-ink-muted'}`}>
              {label}
            </span>
          </Link>
        );
